...
os.makedirs(AKTG_TEMP_DIR, exist_ok=True)  # 确保目录存在
...
class ImageDownloadThread(QThread):
    ...
    def run(self):
        try:
            ...
            # 保存临时文件
            temp_file = tempfile.mktemp(dir=AKTG_TEMP_DIR, suffix=".jpg")
...
class KeyToolsGUI(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"高级密钥工具 {VERSION}")
    ...
    def setup_keygen_tab(self):
        ...
        # 保存为ZIP按钮
        self.btn_save_zip = QPushButton("保存为ZIP")
        self.btn_save_zip.setStyleSheet("background-color: #2196F3; color: white;")
        self.btn_save_zip.clicked.connect(self.save_as_zip)
        self.btn_save_zip.setEnabled(False)
        h_layout.addWidget(self.btn_save_zip)
        
        # 载入密钥包按钮
        self.btn_load_package = QPushButton("载入密钥包")
        self.btn_load_package.setStyleSheet("background-color: #FF9800; color: white;")
        self.btn_load_package.clicked.connect(self.load_key_package_dialog)
        h_layout.addWidget(self.btn_load_package)
    ...
    def setup_rsa_crypto_tab(self):
        ...
        h_layout.addWidget(self.pub_key_text)
        
        # 加载公钥按钮
        self.btn_load_pub = QPushButton("加载公钥")
        self.btn_load_pub.setStyleSheet("background-color: #607D8B; color: white;")
        self.btn_load_pub.clicked.connect(lambda: self.load_key_file(self.pub_key_text, "公钥文件 (*.pem)"))
        h_layout.addWidget(self.btn_load_pub)
        
        group_pub_layout.addLayout(h_layout)
        ...
        h_layout.addWidget(self.priv_key_text)
        
        # 加载私钥按钮
        self.btn_load_priv = QPushButton("加载私钥")
        self.btn_load_priv.setStyleSheet("background-color: #607D8B; color: white;")
        self.btn_load_priv.clicked.connect(lambda: self.load_key_file(self.priv_key_text, "私钥文件 (*.key)"))
        h_layout.addWidget(self.btn_load_priv)
        
        group_priv_layout.addLayout(h_layout)
    ...
    def setup_image_steganography_tab(self):
        ...
        # 添加获取随机图片按钮
        self.btn_random_image = QPushButton("获取随机图片")
        self.btn_random_image.setStyleSheet("background-color: #FF5722; color: white;")
        self.btn_random_image.clicked.connect(self.get_random_image)
        h_layout.addWidget(self.btn_random_image)
    ...
    def setup_file_crypto_tab(self):
        ...
        h_layout.addWidget(self.sym_key_text)
        
        # 加载密钥按钮
        self.btn_load_sym_key = QPushButton("加载密钥")
        self.btn_load_sym_key.setStyleSheet("background-color: #607D8B; color: white;")
        self.btn_load_sym_key.clicked.connect(lambda: self.load_key_file(self.sym_key_text, "密钥文件 (*.txt)"))
        h_layout.addWidget(self.btn_load_sym_key)
        
        group_sym_key_layout.addLayout(h_layout)
        ...
        h_layout.addWidget(self.file_pub_key_text)
        
        # 加载公钥按钮
        self.btn_load_pub_file = QPushButton("加载公钥")
        self.btn_load_pub_file.setStyleSheet("background-color: #607D8B; color: white;")
        self.btn_load_pub_file.clicked.connect(lambda: self.load_key_file(self.file_pub_key_text, "公钥文件 (*.pem)"))
        h_layout.addWidget(self.btn_load_pub_file)
        ...
        h_layout.addWidget(self.file_priv_key_text)
        
        # 加载私钥按钮
        self.btn_load_priv_file = QPushButton("加载私钥")
        self.btn_load_priv_file.setStyleSheet("background-color: #607D8B; color: white;")
        self.btn_load_priv_file.clicked.connect(lambda: self.load_key_file(self.file_priv_key_text, "私钥文件 (*.key)"))
        h_layout.addWidget(self.btn_load_priv_file)
    ...
    def key_generated(self, formatted_key, hex_key, public_key, private_key, key_id):
        ...
        self.current_formatted_key = formatted_key
...
"""
    ...
    def save_as_zip(self):
        """手动保存为ZIP文件"""
        if not self.current_formatted_key:
            QMessageBox.warning(self, "无密钥", "请先生成密钥!")
            return
            
        self.save_key_files(
            self.current_formatted_key,
            binascii.hexlify(self.current_formatted_key.encode()).decode(),
            self.current_public_key,
            self.current_private_key,
            self.current_key_id
        )
    ...
    def save_key_files(self, formatted_key, hex_key, public_key, private_key, key_id):
        ...
        try:
            ...
            with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zipf:
                # 添加格式化密钥文件
                txt_data = f"密钥ID: {key_id}\n"
                txt_data += f"生成时间: {time.strftime('%Y-%m-%d %H:%M:%S')}\n\n"
                txt_data += "===== 格式化密钥 =====\n"
                txt_data += formatted_key
                txt_data += "\n\n===== 完整十六进制 =====\n"
                txt_data += hex_key
                zipf.writestr("key.txt", txt_data)
                
                # 添加公钥文件
                zipf.writestr("key.pem", public_key)
                
                # 添加私钥文件
                zipf.writestr("key.key", private_key)
    ...
    def load_key_package_dialog(self):
        """打开对话框加载密钥包"""
        options = QFileDialog.Options()
        file, _ = QFileDialog.getOpenFileName(self, "选择密钥包文件", "", 
                                             "ZIP文件 (*.zip)", 
                                             options=options)
        if file:
            self.load_key_package(file)
    
    def load_key_package(self, zip_path):
        """加载密钥包文件 - 使用内存操作"""
        try:
            # 读取ZIP文件到内存
            ...
            with zipfile.ZipFile(zip_buffer, 'r') as zip_ref:
                ...
                if "key.txt" in zip_ref.namelist():
                    with zip_ref.open("key.txt") as f:
                        content = f.read().decode('utf-8')
                        # 尝试提取密钥ID
                        if "密钥ID:" in content:
                            key_id = content.split("密钥ID:")[1].split("\n")[0].strip()
                        # 提取格式化密钥
                        if "===== 格式化密钥 =====" in content:
                            formatted_key = content.split("===== 格式化密钥 =====")[1].split("=====")[0].strip()
                
                if "key.pem" in zip_ref.namelist():
                    with zip_ref.open("key.pem") as f:
                        public_key = f.read().decode('utf-8').strip()
                
                if "key.key" in zip_ref.namelist():
                    with zip_ref.open("key.key") as f:
...
密钥ID: {key_id}
加载时间: {time.strftime('%Y-%m-%d %H:%M:%S')}
...
"""
            self.key_result.setText(result)
            self.btn_save_zip.setEnabled(True)
            self.statusBar().showMessage(f"密钥包已加载: {os.path.basename(zip_path)}")
            QMessageBox.information(self, "加载成功", 
                                  f"密钥包加载成功!\n密钥ID: {key_id}")
        except Exception as e:
            logging.error(f"加载密钥包失败: {str(e)}")
            QMessageBox.critical(self, "加载失败", f"加载密钥包时出错:\n{str(e)}")
    
    def load_key_file(self, text_edit, file_filter):
        """加载密钥文件 - 使用内存操作"""
        options = QFileDialog.Options()
        ...
        if file:
            try:
                ...
                self.statusBar().showMessage(f"密钥文件已加载: {os.path.basename(file)}")
            except Exception as e:
                logging.error(f"加载密钥文件失败: {str(e)}")
                QMessageBox.critical(self, "加载失败", f"加载密钥文件时出错:\n{str(e)}")
    ...
    def get_random_image(self):
        ...
        self.image_downloader = ImageDownloadThread()
    ...
    def random_image_error(self, error_msg):
        """随机图片下载错误"""
        QMessageBox.warning(self, "下载失败", error_msg)
        self.stego_status.setText("下载随机图片失败!")
        self.btn_random_image.setEnabled(True)
    ...
    def create_stego_image(self):
        ...
        try:
            ...
            # 取前8字节转换为16进制字符串
            hash_prefix = binascii.hexlify(image_hash[:8]).decode()
            ...
            # 构建特殊识别字符串
            # 格式: "AKTP" + 哈希前缀 + 文件名长度 + 文件名 + 扩展名长度 + 扩展名 + 文件大小
            file_size = len(file_data)
            
            # 编码文件名和扩展名
            file_base_bytes = file_base.encode('utf-8')
            file_ext_bytes = file_ext.encode('utf-8')
            
            # 构建标识符
            identifier = b"AKTP" + hash_prefix.encode() + \
                         struct.pack("B", len(file_base_bytes)) + file_base_bytes + \
                         struct.pack("B", len(file_ext_bytes)) + file_ext_bytes + \
                         struct.pack(">Q", file_size)
            
            # 创建图种: 图片数据 + 识别字符串 + 文件数据
            stego_data = image_data + identifier + file_data
    ...
    def extract_from_stego(self):
        ...
        try:
            ...
            # 查找特殊识别字符串 "AKTP"
            identifier_pos = stego_data.find(b'AKTP', image_end)
            if identifier_pos == -1:
                QMessageBox.critical(self, "提取失败", "未找到隐藏文件标识符!")
                return
                
            # 解析标识符
            # 格式: "AKTP"(4字节) + 哈希前缀(16字节) + 文件名长度(1字节) + 文件名 + 扩展名长度(1字节) + 扩展名 + 文件大小(8字节)
            identifier_start = identifier_pos
            hash_prefix = stego_data[identifier_start+4:identifier_start+20].decode()
            
            # 解析文件名
            name_len = stego_data[identifier_start+20]
            name_start = identifier_start+21
            name_end = name_start + name_len
            file_base = stego_data[name_start:name_end].decode('utf-8')
            
            # 解析扩展名
            ext_len = stego_data[name_end]
            ext_start = name_end + 1
            ext_end = ext_start + ext_len
            file_ext = stego_data[ext_start:ext_end].decode('utf-8')
            
            # 解析文件大小
            file_size = struct.unpack(">Q", stego_data[ext_end:ext_end+8])[0]
            
            # 文件数据开始位置
            file_start = ext_end + 8
            file_end = file_start + file_size
            
            # 验证文件大小
            if file_end > len(stego_data):
                QMessageBox.critical(self, "提取失败", "隐藏文件数据不完整!")
                return
                
            # 提取文件数据
            file_data = stego_data[file_start:file_end]
            
            # 验证图片哈希
            original_image_data = stego_data[:image_end]
            image_hash = hashlib.sha256(original_image_data).digest()
            expected_hash_prefix = binascii.hexlify(image_hash[:8]).decode()
            
            if hash_prefix != expected_hash_prefix:
                QMessageBox.warning(self, "验证失败", 
                                  f"图片哈希验证失败!\n"
                                  f"存储的哈希: {hash_prefix}\n"
                                  f"计算的哈希: {expected_hash_prefix}\n"
                                  "文件可能已被篡改!")
                return
            ...
            while os.path.exists(save_path):
                save_path = os.path.join(save_dir, f"{file_base}_{counter}{file_ext}")
                counter += 1
    ...
    def test_stego_image(self):
        ...
        try:
            ...
            # 查找特殊识别字符串 "AKTP"
            identifier_pos = stego_data.find(b'AKTP', image_end)
            has_identifier = identifier_pos != -1
...
"""
            if has_identifier:
                # 解析标识符
                hash_prefix = stego_data[identifier_pos+4:identifier_pos+20].decode()
                
                # 解析文件名
                name_len = stego_data[identifier_pos+20]
                name_start = identifier_pos+21
                name_end = name_start + name_len
                file_base = stego_data[name_start:name_end].decode('utf-8', errors='replace')
                
                # 解析扩展名
                ext_len = stego_data[name_end]
                ext_start = name_end + 1
                ext_end = ext_start + ext_len
                file_ext = stego_data[ext_start:ext_end].decode('utf-8', errors='replace')
...
标识符信息:
  ...
  文件大小: {file_size:,} 字节
  哈希验证: {'匹配' if hash_prefix == expected_hash_prefix else '不匹配'}
"""
                if hash_prefix == expected_hash_prefix:
                    info += f"""
提示:
...
"""
    ...
    def use_current_formatted_key(self):
        """使用当前生成的格式化密钥"""
        if self.current_formatted_key:
            self.sym_key_text.setPlainText(self.current_formatted_key)
    ...
    def use_current_pub_key_file(self):
        """使用当前生成的公钥（文件加解密）"""
        if self.current_public_key:
            self.file_pub_key_text.setPlainText(self.current_public_key)
            self.statusBar().showMessage("当前公钥已加载!")
        else:
            QMessageBox.warning(self, "无公钥", "请先生成密钥对!")
    ...
    def use_current_priv_key_file(self):
        """使用当前生成的私钥（文件加解密）"""
        if self.current_private_key:
            self.file_priv_key_text.setPlainText(self.current_private_key)
            self.statusBar().showMessage("当前私钥已加载!")
        else:
            QMessageBox.warning(self, "无私钥", "请先生成密钥对!")
    ...
    def encrypt_file(self):
        ...
        try:
            ...
            # 构建加密文件
            encrypted_file = encrypted_key + iv + encrypted_data
    ...
    def decrypt_file(self):
        ...
        try:
            ...
            # 提取各部分数据
            # RSA加密的对称密钥 (256字节)
            encrypted_key = encrypted_file[:256]
            # IV (16字节)
            iv = encrypted_file[256:272]
            # AES加密的数据
            encrypted_data = encrypted_file[272:]
            
            # 导入私钥
            rsa_key = RSA.import_key(priv_key)
            cipher_rsa = PKS1_OAEP.new(rsa_key)
            
            # 解密对称密钥
            sym_key_bytes = cipher_rsa.decrypt(encrypted_key)
            
            # 创建AES解密器
            cipher_aes = AES.new(sym_key_bytes, AES.MODE_CBC, iv)
            ...
            # 保存解密文件
            base_name = os.path.basename(file_path)
            if base_name.lower().endswith('.aktp'):
                base_name = base_name[:-5]  # 移除.aktp扩展名
